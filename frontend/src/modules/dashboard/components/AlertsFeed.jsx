    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
      <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-bold text-gray-900">Payroll System Warnings & AI Feed</h3>
        </div>
        <span
          className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${
            totalAlerts > 0 ? 'bg-amber-50 text-amber-800 border-amber-200' : 'bg-green-50 text-green-800 border-green-200'
          }`}
        >
      ...
      {totalAlerts === 0 ? (
        <div className="py-8 text-center flex flex-col items-center justify-center text-gray-500">
          ...
          <p className="text-sm font-semibold text-gray-700">All Systems Clear</p>
          <p className="text-xs text-gray-400 mt-0.5">No payroll anomalies or missing employee information detected.</p>
        </div>
      ) : (
        <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
          ...
          {payslipWarnings.map((w, idx) => (
            <div
              ...
              className="flex items-start justify-between p-3.5 bg-amber-50/60 border border-amber-200/80 rounded-lg text-xs"
            >
              ...
              <span className="px-2 py-0.5 bg-amber-200/60 text-amber-900 rounded font-semibold text-[10px] uppercase">
                Action Required
              </span>
            </div>
          ))}
          ...
          {aiWarnings.map((w, idx) => (
            <div
              ...
              className="flex items-start justify-between p-3.5 bg-purple-50/60 border border-purple-200/80 rounded-lg text-xs"
            >
              <div className="flex items-start gap-3">
                <div className="p-1.5 bg-purple-100 rounded-md text-purple-700 mt-0.5">
              ...
              <span className="px-2 py-0.5 bg-purple-200/60 text-purple-900 rounded font-semibold text-[10px] uppercase">
                AI Flagged
              </span>
            </div>
          ))}
