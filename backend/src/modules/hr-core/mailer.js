...
// Resend (HTTPS API) is preferred when configured — SMTP (port 587) is commonly
// blocked by ISPs/campus networks, which was silently breaking mail sends for some devs.
if (!config.resend.apiKey && config.smtp.host && config.smtp.user && config.smtp.pass) {
  transporter.verify()
