...
async function updateReview(id, data, reviewerId) { const existing = await getReview(id); if (existing.reviewer_id !== reviewerId) throw new ForbiddenError('Only the reviewer may edit this review'); const enriched = await enrich({ ...data, employee_id: existing.employee_id, reviewer_id: reviewerId }); const updated = await repo.updateReview(id, enriched); if (!updated) throw new ValidationError('Only draft reviews can be edited'); return updated; }
async function changeStatus(id, status, userId) { const review = await getReview(id); if (status === 'submitted' && review.status !== 'draft') throw new ValidationError('Only draft reviews can be submitted'); if (status === 'approved' && review.status !== 'submitted') throw new ValidationError('Only submitted reviews can be approved'); if (status === 'rejected' && !['submitted', 'approved'].includes(review.status)) throw new ValidationError('Only submitted or approved reviews can be rejected'); return repo.changeStatus(id, status, userId); }
...
async function listEmployeeReviews(employeeId, user) { if (user.role === 'employee' && user.employeeId !== employeeId) throw new ForbiddenError('Employees may only view their own reviews'); return listReviews({ employeeId, status: 'approved', limit: 50, offset: 0 }); }
